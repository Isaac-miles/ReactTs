    ...
    <Droppable droppableId='ItemsList'>
      {
        (provided,snapshot)=>(
         ...
         <div className='active_task'>
          <span data-testid='active-task' className="items_heading">Active {items.length > 1 ? 'Tasks' :'Task'}</span>
    ...
    <Droppable droppableId='RemoveItem'>
      {
        (provided,snapshot)=>(
         ...
         <div className='active_task'>
          ...
          {completedItems.length>0 && <span className='count'>{completedItems.length}</span>}
         </div> 
          <span>Drop</span>
